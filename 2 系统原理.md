...
打包文件中有两个`mysystem.rel`文件。刚开始，该文件值存在于`release`目录下，以便让`release_handler`可以提取它。解压之后，`release_handle`会自动将它复制到`release/FIRST`。但有时候打包文件在不包含`release_handler`的情况下解压(如，在解压第一个目标系统是)
